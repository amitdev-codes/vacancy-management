...
// use crocodicstudio\crudbooster\controllers\Controller as CoreController;
use Carbon\Carbon;
use CB;
use CRUDBooster;
...
class BaseCBController extends Controller
{
    ...
    public $is_cancelled_count = false;
    public $is_rejected_count = false;
    public $data_inputan;
    ...
    public $getDelete_view = "crudbooster::default.form";
    public $is_cbLoaded = false;
    public function cbInit()
    {
        //gets overriden when extended
    }
    ...
    public function addCommonAssets()
    {
        ...
        array_push($this->load_css, asset("css/bootstrap-datetimepicker.min.css?"));
        ...
        array_push($this->load_js, asset("js/bootstrap-datetimepicker.js"));
        array_push($this->load_js, asset("js/moment-with-locales.js"));
        array_push($this->load_js, asset("js/moment-timezone-with-data.js"));
        array_push($this->load_js, asset("js/custom.js?" . now()));
    }
    ...
    {
        if ($this->is_cancelled_count == true) {
            $this->table_row_count = DB::table($this->table)
                ->where('is_cancelled', 1)
                ->count();
            return $this->table_row_count;
        }
        if ($this->is_rejected_count == true) {
            $this->table_row_count = DB::table($this->table)
                ->where('is_rejected', 1)
                ->count();
            return $this->table_row_count;
    ...
    public function cbLoader()
    {
        ...
        $this->data['is_cancelled_count'] = $this->is_cancelled_count;
        $this->data['is_rejected_count'] = $this->is_rejected_count;
    ...
    public function getIndex()
    {
        ...
        if (!CRUDBooster::isView() && $this->global_privilege == false) {
            if ($isApplicant) {
                ...
                CRUDBooster::insertLog(trans('crudbooster.log_try_view', ['module' => $module->name]) . ' with applicant id ' . $applicant_id);
            } else {
        ...
        if (Request::get('q')) {
            if ($isApplicant) {
                $encoded_applicant_id = Session::get('applicant_id');
                $applicant_id = Hashids::decode($encoded_applicant_id)[0];
                $desc = "Search for " . Request::get('q') . " in module " . CRUDBooster::getCurrentModule()->name . " with applicant_id " . $applicant_id;
            } else {
                $desc = "Search for " . Request::get('q') . " in module " . CRUDBooster::getCurrentModule()->name . " with user_id " . CRUDBooster::myId();
            }
        ...
        foreach ($data['columns'] as $key => $clm) {
            if ($clm['name'] == "exam_roll_no") {
                $data['columns'][$key]['type_data'] = "int";
            }
        }
        ...
        if (Request::get('filter_column')) {
            $sfDesc = 'Filter on module ' . CRUDBooster::getCurrentModule()->name . ' with ' . $sfDesc . ' with user_id ' . CRUDBooster::myId();
        ...
        // fiscal years
        $data['fiscal_year'] = DB::table('vacancy_ad as vad')
            ->select('vad.fiscal_year_id', 'fy.code')
            ->leftjoin('mst_fiscal_year as fy', 'fy.id', '=', 'vad.fiscal_year_id')
            ->where('fiscal_year_id', Session::get('fiscal_year_id'))
            ->get();

        /** If user is neither superadmin nor admin, filter vacancy ads by fiscal year.
         *  If external applicant, then job opening type is always '1' i.e. 'khulla' only.
         */
        $check_if_nt_staff = Session::get('is_nt_staff');

        // dd($check_if_nt_staff);
        ...
        if ($check_if_nt_staff || (CRUDBooster::myPrivilegeId() == 1) || (CRUDBooster::myPrivilegeId() == 5)) {
            ...
            $data['advertisement'] = DB::table('vacancy_ad as vad')
                ->select('vad.*', 'mj.name_np')
                ->leftjoin('mst_job_opening_type as mj', 'mj.id', '=', 'vad.opening_type_id')
                ->where([['fiscal_year_id', Session::get('fiscal_year_id')], ['vad.is_published', 1], ['vad.is_deleted', false], ['vad.opening_type_id', 1]])
                ->orderBy('id', 'desc')
                ->get();
        }
        ...
        //checking for ntc staff and external
        $encoded_applicant_id = Session::get('applicant_id');
        $applicant_id = Hashids::decode($encoded_applicant_id);
        $applicant_id = $applicant_id[0];

        $isNtStaff = DB::select('select is_nt_staff from applicant_profile where id=:applicant_id', ['applicant_id' => $applicant_id]);
        if (empty($isNtStaff[0]->is_nt_staff)) {
            $data['opening_types'] = DB::select('select id,name_en,name_np from mst_job_opening_type where id=:id', ['id' => '1']);
        } else {
            $data['opening_types'] = DB::table('mst_job_opening_type')->get();
        }

        // dd($data['opening_types']);
        ...
        if ($isApplicant) {
            ...
            $desc = "Acess route " . CRUDBooster::mainpath() . " with user_id " . CRUDBooster::myId();
        }
        ...
        foreach ($this->data_inputan as $ro) {
            ...
            if (@$ro['type'] == 'upload' || @$ro['type'] == 'upload-c') {
                if (Request::hasFile($name)) {
                    ...
                    if ($this->table == 'erp_file_uploads') {
                        $pathmix = 'erp_csv/manual/' . CRUDBooster::myId();
                    } else {
                        $pathmix = Session::get("applicant_id") != null ? Session::get("applicant_id") : CRUDBooster::myId();
                    }

                    $file_path = 'uploads/' . $pathmix . '/' . date('Y-m');
    ...
    public function getAdd()
    {
        ...
        if (!CRUDBooster::isCreate() && $this->global_privilege == false || $this->button_add == false) {
            if ($isApplicant) {
                $encoded_applicant_id = Session::get('applicant_id');
                $applicant_id = Hashids::decode($encoded_applicant_id)[0];
                CRUDBooster::insertLog(trans('crudbooster.log_try_add', ['module' => CRUDBooster::getCurrentModule()->name]) . ' with applicant_id ' . $applicant_id);
            } else {
                CRUDBooster::insertLog(trans('crudbooster.log_try_add', ['module' => CRUDBooster::getCurrentModule()->name]) . " with user_id " . CRUDBooster::myId());
            }
        ...
        if ($isApplicant) {
            $encoded_applicant_id = Session::get('applicant_id');
            $applicant_id = Hashids::decode($encoded_applicant_id)[0];
            $desc = "Acess route " . CRUDBooster::mainpath() . "/add" . ' with applicant_id ' . $applicant_id;
        } else {
            $desc = "Acess route " . CRUDBooster::mainpath() . "/add" . ' with user_id' . CRUDBooster::myId();
        }
    ...
    public function postAddSave()
    {
        ...
        if (!CRUDBooster::isCreate() && $this->global_privilege == false) {
            ...
            CRUDBooster::redirect(CRUDBooster::adminPath(), trans("crudbooster.denied_access") . " with user_id " . CRUDBooster::myId());
        }
    ...
    public function postEditSave($id)
    {
        ...
        if (!CRUDBooster::isUpdate() && $this->global_privilege == false) {
            if ($isApplicant) {
                $encoded_applicant_id = Session::get('applicant_id');
                $applicant_id = Hashids::decode($encoded_applicant_id)[0];
                CRUDBooster::insertLog(trans("crudbooster.log_try_add", ['name' => $row->{$this->title_field}, 'module' => CRUDBooster::getCurrentModule()->name]) . ' with applicant_id ' . $applicant_id);
            } else {
                CRUDBooster::insertLog(trans("crudbooster.log_try_add", ['name' => $row->{$this->title_field}, 'module' => CRUDBooster::getCurrentModule()->name]) . " with user_id " . CRUDBooster::myId());
            }
    ...
    public function getReapply($id)
    {
        ...
        if (!CRUDBooster::isRead() && $this->global_privilege == false || $this->button_edit == false) {
            if ($isApplicant) {
                CRUDBooster::insertLog(trans("crudbooster.log_try_edit", ['name' => $row->{$this->title_field}, 'module' => CRUDBooster::getCurrentModule()->name]) . ' with applicant_id  ' . Session::get('applicant_id'));
            } else {
                CRUDBooster::insertLog(trans("crudbooster.log_try_edit", ['name' => $row->{$this->title_field}, 'module' => CRUDBooster::getCurrentModule()->name]) . " with user_id " . CRUDBooster::myId());
            }
    ...
    public function getDelete($id)
    {
        ...
        if (!CRUDBooster::isDelete() && $this->global_privilege == false || $this->button_delete == false) {
            if ($isApplicant) {
                $encoded_applicant_id = Session::get('applicant_id');
                $applicant_id = Hashids::decode($encoded_applicant_id)[0];
                CRUDBooster::insertLog(trans("crudbooster.log_try_delete", ['name' => $row->{$this->title_field}, 'module' => CRUDBooster::getCurrentModule()->name]) . 'with applicant_id' . $applicant_id);
            } else {
                CRUDBooster::insertLog(trans("crudbooster.log_try_delete", ['name' => $row->{$this->title_field}, 'module' => CRUDBooster::getCurrentModule()->name]) . " with user_id " . CRUDBooster::myId());
            }
    ...
    public function postActionSelected()
    {
        ...
        if ($button_name == 'delete') {
            if (!CRUDBooster::isDelete()) {
                if ($isApplicant) {
                    CRUDBooster::insertLog(trans("crudbooster.log_try_delete_selected", ['module' => CRUDBooster::getCurrentModule()->name]) . '  with applicant_id  ' . Session::get('applicant_id'));
                } else {
                    CRUDBooster::insertLog(trans("crudbooster.log_try_delete_selected", ['module' => CRUDBooster::getCurrentModule()->name]) . " with user_id " . CRUDBooster::myId());
                }
            ...
            if ($isApplicant) {
                $encoded_applicant_id = Session::get('applicant_id');
                $applicant_id = Hashids::decode($encoded_applicant_id)[0];
                CRUDBooster::insertLog(trans("crudbooster.log_delete", ['name' => implode(',', $id_selected), 'module' => CRUDBooster::getCurrentModule()->name]) . ' with applicant_id ' . $applicant_id);
            } else {
                CRUDBooster::insertLog(trans("crudbooster.log_delete", ['name' => implode(',', $id_selected), 'module' => CRUDBooster::getCurrentModule()->name]) . " with user_id " . CRUDBooster::myId());
            }
    ...
    public function getDeleteImage($id = null)
    {
        ...
        if (!CRUDBooster::isDelete() && $this->global_privilege == false) {
            if ($isApplicant || CRUDBooster::myPrivilegeId() == 5) {

                $encoded_applicant_id = Session::get('applicant_id');
                $applicant_id = Hashids::decode($encoded_applicant_id)[0];
                CRUDBooster::insertLog(trans("crudbooster.log_try_delete_image", ['name' => $row->{$this->title_field}, 'module' => CRUDBooster::getCurrentModule()->name]) . '  with applicant_id  ' . $applicant_id);
            } else {
                CRUDBooster::insertLog(trans("crudbooster.log_try_delete_image", ['name' => $row->{$this->title_field}, 'module' => CRUDBooster::getCurrentModule()->name]) . " with user_id " . CRUDBooster::myId());
                CRUDBooster::redirect(CRUDBooster::adminPath(), trans('crudbooster.denied_access'));
            }
        }
        ...
        if ('isApplicant') {
            $encoded_applicant_id = Session::get('applicant_id');
            $applicant_id = Hashids::decode($encoded_applicant_id)[0];
            CRUDBooster::insertLog(trans("crudbooster.log_delete_image", ['name' => $row->{$this->title_field}, 'module' => CRUDBooster::getCurrentModule()->name]) . ' with applicant_id ' . $applicant_id);
        } else {
            CRUDBooster::insertLog(trans("crudbooster.log_delete_image", ['name' => $row->{$this->title_field}, 'module' => CRUDBooster::getCurrentModule()->name]) . " with user_id " . CRUDBooster::myId());
        }
    ...
    public function insert_log($desc, $id, $table)
    {
        try {
            $ap_id = DB::table($table)->select('applicant_id')
                ->where('id', $id)
                ->first();
        } catch (\Exception $e) {
            $ap_id = null;
        ...
        if ($ap_id != null) {
            $change_status = DB::table('vw_applicant_details')
                ->select('applicant_name')
                ->where("applicant_id", '=', $ap_id->applicant_id)
                ->first();
        } else {
            $change_status = $id;
        }
        ...
        DB::table('cms_logs')->insert(
            ['created_at' => $current_time, 'ipaddress' => $ip, 'id_cms_users' => $user, 'description' => $desc, 'change_status' => $change_status->applicant_name]
        );
    }

    public function postEditSaveApplicant($id)
    {
        //check here for BLACKLISTED IP
        $resp = $this->CheckBlacklistedIP();
        if ($resp != false) {
        ...
        $neededUri = str_replace("-save", "", $_SERVER['REQUEST_URI']);
        $id = end(explode('/', $neededUri));
        $hashid = Hashids::encode($id);
        $neededUri = str_replace($id, $hashid, $neededUri);
