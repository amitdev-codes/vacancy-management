...
class FilePromotionNotAcceptedController extends \App\Http\Controllers\BaseCBController
{
    public $ad_id = 0;
    ...
    public function getIndex()
    {
        ...
        $this->cbView('reports.file_promotion_not_accepted', $data);
    ...
    public function getVacancyAdNo()
    {
        ...
        $adno_data = DB::table('vacancy_ad')->select('id', 'ad_title_en')->where([['opening_type_id', 3], ['fiscal_year_id', $fiscal_year_id], ['is_deleted', false]])->get();
        return $adno_data;
    ...
    public function getCandidates($id)
    {
        ...
        $candidate_data = DB::select('select distinct t2.exam_roll_no as roll, t1.*,ms.name_np as current_designation,t1.gender,
            mwl.name_np as work_level,mwo.name_en as working_office,t3.seniority_date_bs,t3.remarks as service_history_remarks,
            ...
            max_medi.name_en as additional_qualification_division
                            ...
                            LEFT JOIN applicant_evaluation_marks aem ON aem.applicant_id = t1.ap_id
...
//         $sql = "select distinct t2.exam_roll_no as roll, t1.* from vw_vacancy_post_applicant_profile t1
        ...
        // $this->makeExcelfile('test',array(
        //     array('data1', 'data2'),
        //     array('data3', 'data4')
        // ));
        // dd($candidate_data->toAray());

        //   $candidatesArray[] = ['id', 'customer','email','total','created_at'];
        //   $candidatesArray[] =  $candidate_data->toArray();

        // foreach ($candidate_data as $key => $value) {
        //     $candidate_data[$key] = (array) $value;
        // }
    ...
    public function downloadPDF(Request $request, $id)
    {
        ...
        //   $pdf = PDF::loadView('selected_candidate\pdf',$data);
    ...
    {
        // $sql = "select * from vw_applicant_details where vacancy_post_id = $id and is_paid = 1";

        // $candidate_data_excel = DB::select(DB::raw($sql));

        //VAARS::prepareExcel('SelectedCandidates', $candidate_data_excel);
...
//         $sql = "select distinct t2.exam_roll_no as roll, t1.* from vw_vacancy_post_applicant_profile t1
        ...
        Excel::create($designation_name->name_np, function ($excel) use ($data) {
            $excel->sheet('New sheet', function ($sheet) use ($data) {
                $sheet->loadView('selected_candidate.file_pormotion_notaccepted_xl_report', $data);
            });
        })->download('xls');
