...
class EvaluationIndividualEmployeeController extends ReportBaseController
{
    ...
    public function getIndividaualIndex($token_number)
    {
        ...
        #applicant_id
        $user_id = DB::table('vacancy_apply')->select('applicant_id')->whereId($token_number)->first();
        $applicant_id = $user_id->applicant_id;
    ...
    WHERE
     ...
     mec.id =:id', ['id' => 5]);
        ...
        // dd($min_days_for_office_incharge);
 ...
 WHERE
     ...
     AND (incharge_date_from_bs >= :seniority_date_bs and incharge_date_to_bs <= :last_fiscal_year_ending_date_bs1)
     order by  incharge_date_from_bs asc',
        ...
        //dd($data['office_incharge'], $last_fiscal_year_ending_date_bs,$seniority_date_bs,$last_fiscal_year_ending_date_ad,$token_number,$last_fiscal_year_ending_date_ad);
...
WHERE
	...
	AND mdd.is_for_internal =:is_for_internal
        ...
        // dd($applicant_id, $least_point, $token_number, $last_fiscal_year_ending_date_bs, $seniority_date_bs, $work_level_id->id);

        //start of education_leave calculation
        $data['leave_calculation'] = DB::select('SELECT
        mald.emp_no,
        mald.applicant_id,
        mald.vacancy_apply_id,
        mald.leave_type_id,
        mlt.name_np AS leave_name,
        mald.date_from_bs AS leave_from_date,
        mald.date_to_bs AS leave_to_date,
        wo.working_office,
        wo.calculation_date_from_bs AS working_office_from_date,
        wo.calculation_date_to_bs AS working_office_to_date,
        wo.seniority_date_bs,
        wo.rate,
        @total_working_days := wo.difference_in_working_days AS total_working_days,
        @total_leave_days :=(DATEDIFF( mald.date_to_bs, mald.date_from_bs )+ 1) AS difference_in_leave_days,
        @total_days_after_leave :=(@total_working_days - @total_leave_days) AS $total_working_days_after_leave,

        ROUND(( wo.rate *( @total_days_after_leave / 365 )), 3 ) AS total_working_points_after_excluding,
	    ROUND(( :least_point2 *( @total_leave_days / 365 )), 3 ) AS total_leave_days_point,


        ROUND((:least_point *( @total_days_after_leave / 365 )), 3 ) AS total_points_after_excluding,
        @working_office_point := ROUND((( wo.rate *( wo.difference_in_working_days ))/ 365 ), 3 ) AS particular_working_office_point,
        @education_leave_point :=
    CASE
            WHEN DATEDIFF( mald.date_to_bs, mald.date_from_bs ) IS NOT NULL THEN
            ROUND(( :least_point1 *(( DATEDIFF( mald.date_to_bs, mald.date_from_bs ) )+ 1 ))/ 365, 3 )
        END education_leave_point,
        ( @working_office_point - @education_absent_point ) AS total_point_after_deduction
    FROM
        merged_applicant_leave_detail mald
        LEFT JOIN mst_leave_type mlt ON mald.leave_type_id = mlt.id
        LEFT JOIN (
        SELECT
            mash.vacancy_apply_id,
            mwo.name_en AS working_office,
            mash.seniority_date_bs,
            date_from_bs,
            date_to_bs,
            @cal_date_from_bs :=
        CASE

            WHEN date_from_bs <=:seniority_date_bs  THEN :seniority_date_bs1 ELSE date_from_bs END calculation_date_from_bs, @cal_date_to_bs := CASE WHEN mash.is_current IS TRUE AND date_to_bs >=:last_fiscal_year_ending_date_bs THEN
            :last_fiscal_year_ending_date_bs1 ELSE date_to_bs
        END calculation_date_to_bs,
        DATEDIFF( @cal_date_to_bs, @cal_date_from_bs ) AS difference_in_working_days,
        eo.geo_category AS varga,
    CASE

            WHEN ap.gender_id = 1 THEN
            gps.rate_male ELSE gps.rate_female
        END rate
    FROM
        merged_applicant_service_history AS mash
        LEFT JOIN mst_working_office AS mwo ON mash.working_office = mwo.id
        INNER JOIN applicant_profile ap ON mash.applicant_id = ap.id
        LEFT JOIN mst_gender mg ON ap.gender_id = mg.id
        LEFT JOIN erp_organization eo ON eo.NAME = mwo.name_en
        LEFT JOIN (
        SELECT
        ...
        FROM
            geo_points_setting
        ) gps ON eo.geo_category = gps.varga
    WHERE
        mash.vacancy_apply_id =:token_number
        AND work_level =:work_level
    ORDER BY
        date_from_bs ASC
        ) AS wo ON mald.vacancy_apply_id = wo.vacancy_apply_id
    WHERE
        mald.applicant_id =:applicant_id
        AND mald.vacancy_apply_id =:token_number1
        AND mald.leave_type_id = 3
        AND mald.date_from_bs >=:seniority_date_bs2
        AND mald.date_to_bs <=:last_fiscal_year_ending_date_bs2
        AND mald.date_from_bs BETWEEN wo.calculation_date_from_bs
        AND wo.calculation_date_to_bs
        AND mald.date_to_bs BETWEEN wo.calculation_date_from_bs
        AND wo.calculation_date_to_bs', [
        ...
        if (!empty($data['leave_calculation'])) {
            ...
            foreach ($data['leave_calculation'] as $keys => $lc) {
                ...
                $point_after_excluding = $lc->total_leave_days_point;
                $particular_office_working_point = $lc->particular_working_office_point;
                $particular_office_working_point_after_excluding = $lc->total_working_points_after_excluding;

                $edu_working_office += $particular_office_working_point;
                $edu_leave_sum += $point_after_excluding;
                $particular_office_point_after_leave += $particular_office_working_point_after_excluding;
            }
            $data['lc_point'] = $edu_leave_sum;
            $data['working_office_point'] = $edu_working_office;
            $data['working_office_point_after_excluding_leave'] = $particular_office_point_after_leave;
        }
