...
use App\Exports\LoksewaExportOpencandidates;
use App\Exports\OpenExamViewExport;
use App\Exports\OpenExamExport;
...
use App\Exports\SelectedCandidates;
use App\Exports\LoksewaSelectedCandidates;

class SelectedCandidatesController extends \App\Http\Controllers\BaseCBController
{
    public $ad_id = 0;
    ...
    public function getDesignationView($id)
    {
        ...
        if($ot==1){
            ...
            #for internal
            $this->cbView('selected_candidate.internal_exam_report', $data);
        }else{
            #for filepromotion
            $this->cbView('selected_candidate.file_promotion_report', $data);
        }
    ...
    public function getCandidates($ad_id, $id)
    {
        ...
        $designation_id = DB::table('vacancy_post')
            ->select('designation_id', 'vacancy_ad_id','id')
            ->where('id', $id)
            ->first();
        ...
        order by
            ...
            $candidate_data = DB::table('vw_vacancy_post_applicant_profile as t1')
                ...
                ->where([['t1.is_paid',true],['t1.vp_id', $id],['t1.total_paid_amount', '<>', ''],['is_rejected',false],['is_cancelled',false]])
...
//     dd($candidate_data);
        ...
        if($ot==1){
            #for khulla
            $this->cbView('selected_candidate.open_exam_report', $data);
        }elseif($ot==2){
            #for internal
            $this->cbView('selected_candidate.internal_exam_report', $data);
        }else{
    ...
    {
        $designation_id =VacancyPost::SelectedCandidateFilter($id)->select('designation_id', 'vacancy_ad_id')->first();
        $opening_type =VacancyAd::filter($designation_id->vacancy_ad_id)->first();
        $designation_name=MstDesignation::query()->filter()->whereId($designation_id->designation_id)->pluck('name_en');
        $filename=$designation_name[0].'.xlsx';

        
        $designation_name = DB::table('mst_designation')->select('name_np')
            ->where('id', $designation_id->designation_id)
            ->first();

           $filename=$designation_name->name_np.'.xlsx';



        if ($opening_type->opening_type_id === 1) {
            return Excel::download(new OpenExamExport($id), $filename);
        } elseif ($opening_type->opening_type_id == 2) {
    ...
    FROM
        ...
        ini_set('memory_limit', '-1');
           ...
           $reportName=$designation_name->name_np.'.xlsx';


        if ($opening_type->opening_type_id == 1) {
            return Excel::download(new LoksewaExportOpencandidates($id),$reportName);
        } elseif ($opening_type->opening_type_id == 2) {
