...
class ApplicantRatioWiseReportController extends ReportBaseController
{
    public function cbInit()
    {
        # START CONFIGURATION DO NOT REMOVE THIS LINE
        // $this->table               = "vacancy_apply";
        $this->report_name = "Applicant Ratio Wise Count";
        $this->title_field = "id";
        $this->limit = 20;
        ...
        mw.code as work_code,
          d.name_en AS designation,
          coalesce(sum(vp.apanga_seats),0) AS total_apanga,
          coalesce(sum(vp.dalit_seats),0) AS total_dalit,
          coalesce(sum(vp.janajati_seats),0) AS total_janajati,
          coalesce(sum(vp.madheshi_seats),0) AS total_madhesi,
          coalesce(sum(vp.mahila_seats),0) AS total_mahila,
          coalesce(sum(vp.open_seats),0) AS total_open,
          coalesce(sum(vp.remote_seats),0) as total_remote,
          sum(vp.total_req_seats) AS total_req,
          sum(va.is_female) AS female,
          sum(va.is_dalit) AS dalit,
          sum(va.is_madhesi) AS madhesi,
          sum(va.is_handicapped) as handicapped,
          sum(va.is_open) AS open,
          sum(va.is_janajati) AS janajati,
          sum(va.is_remote_village) as remote_village,vacancy_ad_id,va.designation_id as designation_id
      FROM
          vacancy_apply AS va
      Left JOIN vacancy_post vp ON va.vacancy_post_id = vp.id
      LEFT JOIN mst_designation d ON va.designation_id = d.id
      LEFT JOIN mst_work_level mw on d.work_level_id = mw.id
      GROUP BY
          d.name_en,
          work_code,vacancy_ad_id,designation_id
        ) as count_sql";
        # START COLUMNS DO NOT REMOVE THIS LINE
        $this->col = array();
        $this->col[] = array("label" => "Level", "name" => "work_code");
        $this->col[] = array("label" => "Designation", "name" => "designation");
        $this->col[] = array("label" => "Total Open", "name" => "total_open");
        $this->col[] = array("label" => "Applied Open", "name" => "open");
        $this->col[] = array("label" => "Total Female", "name" => "total_mahila");
        $this->col[] = array("label" => "Applied Female", "name" => "female");
        $this->col[] = array("label" => "Total Dalit", "name" => "total_dalit");
        $this->col[] = array("label" => "Applied Dalit", "name" => "dalit");
        $this->col[] = array("label" => "Total Apanga", "name" => "total_apanga");
        $this->col[] = array("label" => "Applied Apanga", "name" => "handicapped");
        $this->col[] = array("label" => "Total Madhesi", "name" => "total_madhesi");
        $this->col[] = array("label" => "Applied Madhesi", "name" => "madhesi");
        $this->col[] = array("label" => "Total Janajati", "name" => "total_janajati");
        $this->col[] = array("label" => "Applied Janajati", "name" => "janajati");
        $this->col[] = array("label" => "Total Remote", "name" => "total_remote");
        $this->col[] = array("label" => "Applied Remote", "name" => "remote_village");
        $this->col[] = array("label" => "Total Required Seats", "name" => "total_req");
