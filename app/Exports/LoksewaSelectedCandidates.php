...
class LoksewaSelectedCandidates implements FromView,WithHeadings,ShouldAutoSize,WithEvents
{
    ...
    public function view(): View
    {
        ...
        order by
            ...
            $candidate_data = DB::select('select distinct t2.exam_roll_no as roll,
             t1.*,mg.name_np as gender
            from vw_vacancy_post_applicant_profile t1
            left join vacancy_exam t2 on t1.vacancy_apply_id = t2.vacancy_apply_id
            left join mst_gender mg on t1.gender =mg.id
        ...
        $intro_data = DB::select('SELECT mws.name_np as samuha,mwsg.name_np as upsamuha,
        va.notice_no,va.date_to_publish_bs,vp.ad_no, va.ad_title_en, vp.designation_id, wl.name_en as work_level, md.name_en as designation,
    ...
    FROM
        ...
        left join mst_work_service mws on md.work_service_id=mws.id	
        LEFT JOIN mst_work_service_group mwsg ON md.service_group_id = mwsg.id
        ...
        if ($opening_type->opening_type_id == 1) {
            // return view("selected_candidate.xl_report",$data);
            return view("selected_candidate.loksewa_open_excel_report",$data);
        } elseif ($opening_type->opening_type_id == 2) {
            return view("selected_candidate.loksewa_internal_excel_report",$data);
        }else{
            return view("selected_candidate.loksewa_file_promotion_excel",$data);
        }
