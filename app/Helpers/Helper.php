...
class Helper
{
    ...
    private function validate_en($year, $month, $day)
    {
        ...
        if ($day < 1 || ($day > cal_days_in_month(CAL_GREGORIAN, $month, $year))) {return 'Invalid day Range';}
        return true;
    }
    //Convert AD to Bs
    public static function get_nepali_date($year, $month, $day)
    {

        $data=new Helper;
        $validate = $data->validate_en($year, $month, $day);
        if ($validate !== true) {
            die($validate);
        }

        $date = $year . '-' . $month . '-' . $day;
        $dayname = $data->get_week_ne($year, $month, $day);
        $date_start = date_create($data->firstday_en);
        $date_today = date_create($date);
        $diff = date_diff($date_start, $date_today, true);
        $days = $diff->format("%a");
        $arr = '0';
        $mm = '';
        for ($i = $data->start_ne; $i < $data->end_ne; $i++) {
            $arr += array_sum($data->nepali_length[$i]);

            if ($arr > $days) {
                $year = $i;

                $count_previous = $arr - array_sum($data->nepali_length[$i]);
                $year_previous = $i - 1;
                for ($j = 0; $j < 12; $j++) {
                    $count_previous += $data->nepali_length[$i][$j];
                    if ($count_previous > $days) {
                        $month = $j + 1; //Even I don't Know Why should I add 1 :p
                        $daysss = $count_previous - $days;
                        $dayss = ($data->nepali_length[$i][$j] - $daysss) + 1;
                        break;
        ...
        $date['y'] = $year;
        $date['m'] = $month;
        $date['M'] = $data->month_name[$month - 1];
        $date['d'] = $dayss;
        $date['l'] = $dayname;
    ...
    public function get_eng_date($year, $month, $day)
    {
     ...
     public static function money_format_nep($number, $is_unicode = true)
    {
      if (strstr($number, "-")) {
      ...
      $paise = @$split_number[1];
        ...
        if ($is_unicode) {
            $amount=Helper::get_nep($negative . $formatted_rupee . $formatted_paise);
            return $amount;
      ...
      public static function diff_in_days($date_from,$date_to):int{
          return Carbon::parse($date_from)->diffInDays(Carbon::parse($date_to));
      }
    public static function daysInPeriod($days){
        $duration_year=$days/365;
        $total_year=(int)$duration_year;
        $remaining_duration=$duration_year-$total_year;

        $duration_month=$remaining_duration*12;
        $total_month=(int)$duration_month;

        $remaining_duration_days=$duration_month-$total_month;
        $days=(int)($remaining_duration_days*30);

        if($total_year>=1){
            $year=$total_year.'years';
        }else{
            $year='-';
        }
        if($total_month>=1){
            $month=$total_month.'months';
        }else{
            $month='-';
        }
        if($days>=1){
