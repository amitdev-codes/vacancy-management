						...
						<div class="panel-body child-form-area">
							...
							<script type="text/javascript">
								...
								function validateForm{{$name}}() {
									...
									$('#panel-form-{{$name}} input[type=number]' ).each(function() {
										var v = parseInt($(this).val());
										if(v>parseInt($(this).attr('max'))){
											sweetAlert("{{trans('Please look for the max number that can be added.')}}","{{trans('crudbooster.please_complete_the_form')}}","warning");
											is_false += 1;
										}
									})
									if(window.location.href.indexOf("/add") > -1) {
									if(checkChildTableData()==false){
										sweetAlert("{{trans('This center is already added.')}}","{{trans('Look for the data already added.')}}","warning");
										is_false += 1;
								...
								function checkChildTableData(){
									var check=true;
									var center_id=$('.input-id','#examcenterscenter_id').val();
										centers={};
										i=0;
										$('#table-{{$name}} > tbody  > tr').each(function() {
