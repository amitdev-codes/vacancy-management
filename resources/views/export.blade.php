...
<head>
	<title>{{Request::input('filename')}}</title>
	<meta charset="utf-8">
	<style>
...
foreach ($columns as $col) {
	...
	if (@$col['image']) {
		...
		$pic_small = $pic;
		if (Request::input('fileformat') == 'pdf') {
			echo "<td><a data-lightbox='roadtrip' rel='group_{{$table}}' title='$col[label]: $title' href='" . $pic . "'><img class='img-circle' width='65px' height='65px' src='" . $pic_small . "'/></a></td>";
