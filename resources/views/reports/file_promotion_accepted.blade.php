...
<div class="btn-group">
 <button type="button" class="btn btn-primary dropdown-toggle nepali_td" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{{$combo_title}}
	...
	<ul class="dropdown-menu">
		@if($adno_data) 
		  @foreach($adno_data as $ad)
		  <li><a class="dropdown-item nepali_td" href="{{route('filepromotion_designation_view',['id'=>$ad->id])}}">{{$ad->ad_title_en}}</a></li>
		 @endforeach
	    @endif
	</ul>
</div>
...
<div class="btn-group">
	...
	<li>
		<a class="dropdown-item" href="{{route('filepromotion_getCandidates',['ad_id'=>$ad->id,'id'=>$value->post_id])}}">{{$value->designation}}</a>
	</li>
		@endforeach
		 @endif
	</ul>
...
<div class="btn-group">
	@if($candidate_data)
				<a href="../file_promotion/export/{{request()->route('id')}}">
	...
	<header>
		<div class="left header panel panel-primary">
			...
			<div class='panel-footer'>
				<p class="nepali_td">आन्तरिक मुल्याङ्कन बदुवा तर्फका उम्मेदवारहरुको स्वीकृत नामावली</p>
			</div>
		</div>
	</header>
			...
			<?php $count = 1;
				...
				<tr>
					...
					@if($opening_type_id!=3)
					 <td>{{$value->roll}}</td>
					 @if(isset($value->roll))
					 <td><a href="/admin/admit_card/user/{{$value->token_number}}" target="_blank" style="margin-left:10px;" class="btn btn-xs btn-info">E-Admit Card</a></td>
					 @else
					 <td>--</td>
					@endif
					@endif					 
					...
					@endif
					<td>{{$value->gender}}</td>
					<td>{{$value->applicant_name_en}}<br>{{$value->applicant_name_np}}</td>
					<!-- <td></td> -->
